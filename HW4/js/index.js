...
function waitTransactionStatus() {
	$('#accountStatus').html('帳戶狀態 <b style="color: #000eff">(等待交易驗證中...)</b>')
}
...
let BuyTimeDepositButton = $('#BuyTimeDepositButton');
let BuyTimeDepositCount = $('#BuyTimeDepositCount');
let BuyTimeDepositAmount = $('#BuyTimeDepositAmount');

let InterruptInput = $('#InteruptInput');
let InteruptCountButton = $('#InteruptCountButton');

// 當按下購買定存按鍵時
BuyTimeDepositButton.on('click', async function () {
console.log("first");
console.log(BuyTimeDepositAmount.val());
    console.log(BuyTimeDepositCount.val());
    ...
    waitTransactionStatus();
    // 定存
    bank.methods.BuyTimeDeposit(parseInt(BuyTimeDepositCount.val(), 10) ).send({
        from: nowAccount,
        gas: 3400000,
        value: web3.utils.toWei(BuyTimeDepositAmount.val(), 'ether')
    })
        .on('receipt', function (receipt) {
        	let EventLog = receipt.events.BuyTimeDepositEvent.returnValues;
            log(EventLog, '定存成功')
            $('#ShowBuyTimeDepositAmount').text('金額: ' + EventLog.value)
            $('#ShowBuyTimeDepositCount').text('期數: ' + EventLog.count)
...
// 當按下到期按鍵時
InteruptCountButton.on('click', async function () {
    if (bankAddress == "") {
        return;
    }
    ...
    // 定存
	let InteruptCount = InterruptInput.val()
    bank.methods.TimeDepositExpired(parseInt(InteruptCount, 10)).send({
        from: nowAccount,
        gas: 3400000,
    })
        .on('receipt', function (receipt) {
            let EventLog = receipt.events.TimeDepositExpiredEvent.returnValues;
            log(EventLog, '提領成功！本金與利息已存入銀行帳戶')
            let count =  parseFloat($('#ShowBuyTimeDepositCount').text().replace(/[^0-9]/g, ''), 10)

            $('#ShowBuyTimeDepositAmount').text('金額: ')
            $('#ShowBuyTimeDepositCount').text('期數: ')
...
let EndContract = $('#EndContract')

EndContract.on('click', async function () {
    let count = parseInt($('#ShowBuyTimeDepositCount').text().replace(/[^0-9]/g, ''), 10)
    InterruptInput.attr('value', count)
    InteruptCountButton.trigger('click');
})
